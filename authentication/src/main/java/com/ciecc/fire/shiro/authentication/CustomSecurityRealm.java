package com.ciecc.fire.shiro.authentication;
...
/***
 * 一个小例子，说明如何创建一个关于Authentication(认证)和Authorization(授权)的JDBC领域
 * 
 * @author fire
 *
 */
public class CustomSecurityRealm extends JdbcRealm {
	 ...
	 */
	protected static final String DEFAULT_AUTHENTICATION_QUERY = "SELECT password FROM user WHERE account = ?";
	...
	protected static final String DEFAULT_USER_ROLES_QUERY =
			// from user self
			"SELECT r.role_name FROM user_roles ur " + "JOIN user u USING(user_id) " + "JOIN roles r USING(role_id) "
					+ "WHERE u.username = ? = ? " + "UNION "
					// from his groups
					+ "SELECT r.role_name FROM user_roles_groups urg " + "JOIN user u ON(urg.user_id=u.user_id) "
					+ "JOIN roles_groups_roles rgr USING(roles_group_id) " + "JOIN roles r ON(rgr.role_id = r.role_id) "
					+ "WHERE u.username = ? ";
